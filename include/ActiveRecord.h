...
#include <internal/ActiveRecordDataMapper>

template <class T, typename Identity = unsigned>
class ActiveRecord
{
public:
	ActiveRecord() : saved(false) {
	}
	
	virtual ~ActiveRecord() {
	}
	
	typedef internal::ActiveRecordDataMapper<T, Identity> DataMapper;
...
#define DECLARE_PROPERTY(name, type) INTERN_CONST_PROPERTY(name, type)\
	void name(const QVariant& name) { _##name = name.value<type>(); saved = false; };

#define DECLARE_RELATION(name, type) INTERN_PROPERTY(name, type*)\
